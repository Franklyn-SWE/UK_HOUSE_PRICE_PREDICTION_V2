...
def add_target_encoding(train_df: pd.DataFrame, valid_df: pd.DataFrame, 
    ...
    for col in ["postcode", "town", "district", "county", "property_type"]:
        ...
        encoding_map[col] = dict(zip(col_means[col], col_means["smoothed_mean"]))
    
    # Apply encoding to all datasets
    for df_set in [train_df, valid_df, test_df]:
        df_set["postcode_mean_price"] = df_set["postcode"].map(
            encoding_map["postcode"]
        ).fillna(train_df["price"].mean())
...
def main():
    ...
    model = CatBoostRegressor(
        ...
        depth=10,                  # Increased from 8 for more complex patterns
        learning_rate=0.03,        # Decreased for better generalization
        l2_leaf_reg=5,             # NEW: L2 regularization
        bagging_temperature=0.5,   # NEW: Bayesian bootstrap
        random_strength=0.5,       # NEW: Score randomization
        eval_metric="RMSE",
        random_seed=42,
